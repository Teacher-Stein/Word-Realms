// ---------------------------------------------------------------------------
// THE CONTENT RULES — shared by the terminal checker and the browser page.
//
// These are the checks that catch the mistakes a human makes writing a hundred
// questions in a sitting. Every one of them is here because it happened.
//
// This file exists so there is exactly ONE copy of them. It is loaded two ways:
//
//   tools/check-content.html      a page you open in a browser. No install.
//   tools/tests/check_content.js  the same rules from a terminal, for CI.
//
// A second copy would drift within a version, and then the two would disagree
// about whether a question bank was sound - which is worse than having one.
// ---------------------------------------------------------------------------
(function (root) {
  "use strict";

  function checkContent(REALMS, CONFIG) {
    const problems = [], notes = [];
    const bad  = (msg) => problems.push(msg);
    const note = (msg) => notes.push(String(msg));
    ...
    function checkBank(realmName, standard, elite) {
      ...
      note(`standard ${standard.length} · elite ${elite.length} · ` +
      ...
      if (stdKeys.size > CONFIG.BOSS_MAX_QUESTIONS)
        note(`note             : ${stdKeys.size} keys but the Boss is ` +
  ...
  root.checkContent = checkContent;
